    ...
    def plan_trip(
        ...
        try:
            ...
            if isinstance(restaurants, pd.DataFrame):
                ...
                logger.info(f"Restaurant data format: {type(restaurants)}")
        except Exception as e:
            logger.error(f"Error fetching restaurants: {str(e)}")
            restaurants = []
        
        # Format flight and hotel info for the itinerary generator
        ...
        try:
            # Try using your existing generate_itinerary function from LLMchat.py first
            try:
